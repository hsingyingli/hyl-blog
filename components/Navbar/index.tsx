    ...
    <div className='backdrop-blur w-full fixed top-0 left-0 z-50 dark:bg-black bg-amber-50'>
      <div className='max-w-screen-2xl mx-auto flex gap-7 items-center justify-between p-3'>
        ...
        <div className="flex gap-3">
          <div className='flex gap-5 text-lg item-center justify-center mr-10'>
            <Link href='/' passHref><a className='p-1 rounded-md' style={{ backgroundColor: path === '/' ? '#14B8A6' : 'transparent' }}>Home</a></Link>
            {session?.user &&
              <Link href='/posts'><a className='bg-red-500 p-1 rounded-md' style={{ backgroundColor: path === '/posts' ? '#14B8A6' : 'transparent' }}>Posts</a></Link>}
          </div>
          <ThemeToggleButton />
          <DropdownMenu />
        </div>
